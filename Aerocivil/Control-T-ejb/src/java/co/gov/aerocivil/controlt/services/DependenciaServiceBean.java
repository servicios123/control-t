...
public class DependenciaServiceBean implements DependenciaService {
    ...
    @Override
    public List<Dependencia> getLista(Dependencia dependencia) {

        Query query = createQueryFilter(dependencia);
        try {
            return (List<Dependencia>) query.getResultList();
        } catch (NoResultException nre) {
            nre.printStackTrace();
    ...
    private Query createQueryFilter(Dependencia dependencia) {
