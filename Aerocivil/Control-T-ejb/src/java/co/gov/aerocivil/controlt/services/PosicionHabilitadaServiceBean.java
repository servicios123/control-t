...
public class PosicionHabilitadaServiceBean implements PosicionHabilitadaService {
    ...
    @Override
    public List<PosicionHabilitada> getLista(PosicionHabilitada posicionHabilitada) {

        try {
            Query query = createQueryFilter(posicionHabilitada);
            return (List<PosicionHabilitada>) query.getResultList();
        } catch (NoResultException nre) {
            nre.printStackTrace();
            return null;
    ...
    private Query createQueryFilter(PosicionHabilitada posicionHabilitada) {
