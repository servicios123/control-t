...
public class DiarioSenalServiceBean implements DiarioSenalService {
    ...
    @Override
    public List<DiarioSenal> getLista(DiarioSenal diarioSenal) {
        try {
            Query query = createQueryFilter(diarioSenal);
            return (List<DiarioSenal>) query.getResultList();
        } catch (NoResultException nre) {
            nre.printStackTrace();
            return null;
    ...
    private Query createQueryFilter(DiarioSenal diarioSenal) {
